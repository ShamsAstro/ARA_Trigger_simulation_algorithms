...
OUT_DIR = Path("TOT_efficiency_plots_single_Noenv_tot2")
...
for min_tot, thr_value in TOT_thresholds.items():
    ...
    plt.grid()
    plt.legend()
    plt.savefig(OUT_DIR / f"Efficiency_TOT{min_tot}_Thr{thr_value}_env2.png")
    plt.close()
    ...
    plt.grid()
    plt.savefig(OUT_DIR / f"TOT_vs_SNR_TOT{min_tot}_Thr{thr_value}_env2.png")
    plt.close()
