...
wf_10 = channels[:10].sum(axis=0)
noise_10 = noise[:10].sum(axis=0)

wf_20 = channels[:20].sum(axis=0)
noise_20 = noise[:20].sum(axis=0)
...
axes[0].set_title("Signal in Uncorrelated Gaussian Noise: Single Channel vs Summed Channels",
                  fontsize=18, pad=14)
...
fig.savefig('Hello1', dpi=200, bbox_inches="tight")
plt.close(fig)