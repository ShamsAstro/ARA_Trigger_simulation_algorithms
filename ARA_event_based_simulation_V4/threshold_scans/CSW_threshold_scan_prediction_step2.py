...
# CONFIG
# ─────────────────────────────────────────────────────────────────────────────
IN_JSON = Path("Test_threshold_scan_CSW_2windows_30min_fast.json")

LABEL = "PURE noise CSW, N_segments = 1"
EVENT_NS = 170.0 *2
TARGET_HZ = 5.0

FIT_START_CANDIDATES = list(range(11, 20, 2))

X_AXIS_START = None
Y_AXIS_TOP = 1e9
Y_AXIS_BOTTOM = 0.1

OUT_DIR = Path("plots_CSW_Nsegment_1_threshold_scan_CSW")
OUT_DIR.mkdir(exist_ok=True)

SUMMARY_JSON = OUT_DIR / "summary_CSW_Nsegment_1_analysis_parallel.json"
...
def main():
    ...
    if threshold_5hz is not None and np.isfinite(threshold_5hz):
        ...
        text = f"{threshold_5hz:.3f}"
        if threshold_5hz_err is not None and np.isfinite(threshold_5hz_err):
            text = f"{threshold_5hz:.3f} ± {threshold_5hz_err:.3f}"
    ...
    out_plot = OUT_DIR / "threshold_scan_Nsegment_1.png"
