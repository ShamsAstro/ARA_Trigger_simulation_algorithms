...
def generate_pulse_with_delay(pulse_v, pulse_t, STEP, simulation_index_duration,
                              amplitude_scale, start_time, delay_ns):
    """
    Same as generate_pulse, but applies a time delay (ns).

    Positive delay  -> shift right (later in time)
    Negative delay  -> shift left (earlier in time)

    Anything outside bounds is clipped.
    """

    # Convert delay from ns → samples
    ...
    # Apply delay
    start_index_shifted = start_index + delay_samples

    # Build extended pulse (same idea as yours)
    zeros_array = np.zeros(len(pulse_v), dtype=pulse_v.dtype)
    pulse_v_zeros = np.concatenate((pulse_v, zeros_array))

    # Create indices (same structure, just shifted)
    pulse_indices = np.linspace(start_index_shifted,
                                start_index_shifted + len(pulse_v),
                                simulation_index_duration,
                                dtype=int)

    # Clip indices to valid range (this handles cutting automatically)
    pulse_indices = np.clip(pulse_indices, 0, len(pulse_v_zeros) - 1)

    # Generate signal
    signal = pulse_v_zeros[pulse_indices] * amplitude_scale
...
def make_full_signal_with_delay(impulse_json_path, SIMULATION_DURATION_NS, SAMPLING_RATE, NOISE_EQUALIZE,
    ...
    pulse = generate_pulse_with_delay(pulse_voltage, pulse_time, time_step, simulation_duration_samples, amplitude_scale, start_time, pulse_delay_ns)
